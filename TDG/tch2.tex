...
El esquema de Godunov resuelve el problema de Riemann sobre cada celda del dominio discretizado y genera el flujo numérico apropiado entre cada interfaz del dominio, para cada instante de tiempo. Sin embargo, el valor obtenido para cada celda es el promedio de la función $u_i$ en dicho intervalo; y esto produce error numérico. Por otro lado, la resolución del problema de Riemann es usualmente una tarea realizable, pero que puede resultar costosa y típicamente se necesita de algún otro método numérico para resolver ecuaciones algebraicas no lineales.