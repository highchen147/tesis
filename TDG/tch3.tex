...
\end{equation}
donde $c_p$ es la capacidad calorífica específica del gas a presión constante. A partir del anterior resultado se define la \textbf{entalpía} específica $\mathrm{h}$ \footnote{No confundir con $h$ definida en \ref{eq:def-h} como el tamaño de cada celda del dominio.}:
\begin{equation}
	\mathrm{h} \equiv e + \frac{p}{\rho}.
	\label{eq:entalpia-1}
\end{equation}
Para un gas politrópico se considera a $c_p$ como constante, por lo que integrando la ecuación (\ref{eq:entalpia-temp-dif}) se obtiene otra expresión para la entalpía,
\begin{equation}
	\mathrm{h} = c_p T.
	\label{eq:entalpia-2}
\end{equation}
Por otro lado, de acuerdo a la ley del gas ideal se tiene
\begin{equation}
	c_p - c_v = \mathcal{R}.
...
\end{equation}
De tal manera que la energía total del gas queda como
\begin{equation}
